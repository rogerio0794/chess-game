...
public class UI {
	...
	// Ler uma posição do xadrex (letra + numero)
		public static ChessPosition readChessPosition(Scanner sc) {
			...
			try {
				...
				// Recortar meu string a partir da posição 1 e converter para inteiro
				int row = Integer.parseInt(s.substring(1));
				return new ChessPosition(column, row);			
			} 
			catch (Exception e) {
				throw new InputMismatchException("Erro na leitura da posição de xadrez. Valores validos são de a1 até h8");
			}
		}

	// Imprimir as peças todas do tabuleiro
	...
	// Imprimir as peças todas do tabuleiro e identificar para onde ela pode se mover
	...
	// Imprimir a partida (não só o tabuleiro), exibir o turno e jogador atual
	...
	private static void printCapturedPieces ( List<ChessPiece> capturedPieces) {
		...
		// expressão lambda, stream e filtro
		// Vamos filtrar da lista todo mundo cuja a cor é branca
	...
	// Imprimir apenas uma peça e as posições que ela pode se mover (background)
