...
public class ChessMatch {
	...
	private ChessPiece enPassantVulnerable; // Pegar a peça que está vulneravel para a jogada especial Passant
	...
	public ChessPiece performChessMove(ChessPosition  sourcePosition, ChessPosition targetposition) {
		...
		if (movedPiece instanceof Pawn) {
			if ((movedPiece.getColor() == Color.WHITE && target.getRow() == 0)  || (movedPiece.getColor() == Color.BLACK && target.getRow() == 7)) {
				...
				// Trocando para rainha por padrão
		...
		// En passant
		// Testar se a peça movida é um peão que moveu duas casas
		// Se sim ele será marcado como vulneravel para en passant
	...
	public ChessPiece replacePromotedPiece(String type) {
		...
		// Pegando a posição da peça que será promovida
		Position pos = promoted.getChessPosition().toPosition();
		
		// Removendo do tabuleiro a mesma
		Piece p = board.removePiece(pos);
		piecesOnTheBoard.remove(p); // Removendo da lista de peças presentes no tabuleiro
		
		// Instânciando uma nova peça
		ChessPiece newPiece = newPiece(type, promoted.getColor());
		board.PlacePiece(newPiece, pos); // Colocando no tabuleiro
		piecesOnTheBoard.add(newPiece);  // Adicionando na lista de peças presentes no tabuleiro
